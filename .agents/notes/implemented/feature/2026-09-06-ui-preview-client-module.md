...
- The self-contained `PreviewFileCache` reads `readPreviewFile` and scopes URLs to the Session binding; it does not touch `ui-conversation`'s image cache. A produced PPTX is converted to a PNG thumbnail in `preview_files` (LibreOffice `soffice` -> PDF, poppler `pdftoppm` -> PNG) and registered as an image preview; when the conversion stack is absent the tool degrades to the original PPTX (download link).
...
- **Produced files auto-register**: the fs `write`/`edit` tools fold a successful mutation into the result's `previews` meta, so the renderer shows the preview row without the model calling `preview_files` explicitly. `mediaTypeOf` moved to the `dsh-preview` contract so both the tool and the fs tools share it (avoiding a tool-preview <-> fs cycle); registration reads the optional `fileAttachments` service via `ctx.get`, so the fs tools keep working where the store is absent.
...
## Follow-up (deferred): ui-conversation domain layering

`verify-client-domain-graph` reports three pre-existing violations — `skeleton/InputBar` imports `input/editor/*` (ComposerContentEditable, DecoratorPortals, registerComposerKeymap). The domain-graph rule forbids sibling-domain imports and only exempts `contract/` (type layer) or `src/client/` root files (`domainOf` returns `''` for root, so a subdirectory becomes its own domain and `contract/` cannot import one). A clean fix therefore requires one of: (a) flatten the editor family to `src/client/` root, (b) extract a dedicated shared composer-editor module, or (c) have the `input` domain expose the composer editor to the shell via a slot. Deferred as a separate ui-conversation refactor.
