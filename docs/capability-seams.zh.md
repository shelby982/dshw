...
flowchart LR
  ...
  svc_fileAttachments["ctx.fileAttachments<br/>Durable general-file attachment storage"]
  pkg_tool_preview["tool-preview"]
  pkg_llm["llm"]
  ...
  pkg_attachment --> svc_fileAttachments
  pkg_attachment_local --> svc_attachments
  pkg_attachment_local --> svc_fileAttachments
  ...
  svc_fileAttachments --> pkg_api_session_controller
  svc_fileAttachments --> pkg_tool_preview
...
| `ctx.fileAttachments` | `seam` | [`attachment`](../packages/attachment/attachment) | [`attachment-local`](../packages/attachment/attachment-local) | [`api-session-controller`](../packages/api/session-controller), [`tool-preview`](../packages/preview/tool-preview) | - | 通用文件附件像图片一样内容寻址，但持有的是产物文件（不限于图片）；previewFile RPC 把记录的引用授权回产出它的会话。 |
