...
| `ctx.fileAttachments` | `seam` | [`attachment`](../packages/attachment/attachment) | [`attachment-local`](../packages/attachment/attachment-local) | [`api-session-controller`](../packages/api/session-controller), [`tool-preview`](../packages/preview/tool-preview) | - | General-file attachments are content-addressed like images but hold produced files (not just images); the previewFile RPC authorizes a recorded reference back to the session that produced it. |
